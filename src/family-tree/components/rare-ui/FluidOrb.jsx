...
function hexToRgb(hex) {
  ...
  if (h.length !== 6 || Number.isNaN(n)) return [0.9, 0.4, 0.08]; // Default warm Medida amber
...
export default function FluidOrb({
  ...
  useEffect(() => {
    ...
    const mediaReduce = window.matchMedia?.('(prefers-reduced-motion: reduce)')?.matches || false;
    const reduce = isReducedMotion || mediaReduce;
    const start = performance.now();
    let raf = 0;

    const render = (now) => {
      gl.uniform1f(uTime, reduce ? 0.5 : (now - start) / 1000);
